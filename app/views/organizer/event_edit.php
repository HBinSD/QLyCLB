    ...
    <form action="index.php?page=event-edit" method="POST">
        ...
        <div style="display: flex; flex-direction: column; gap: 15px;">
            <div>
                ...
                <input type="text" name="club_id" value="<?= htmlspecialchars($event->getClubId()) ?>" readonly style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 8px; box-sizing: border-box; background: #f1f5f9; color: #64748b; cursor: not-allowed;">
            </div>
...
<div style="display: flex; gap: 15px;">
    <div style="flex: 1;">
        <label style="display: block; font-weight: bold; margin-bottom: 6px; color: #334155;">Thời Gian Bắt Đầu (*)</label>
        <input type="datetime-local" name="start_time" value="<?= isset($event) && $event->getStartTime() ? $event->getStartTime()->format('Y-m-d\TH:i') : '' ?>" required style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 8px; box-sizing: border-box;">
    </div>
    <div style="flex: 1;">
        <label style="display: block; font-weight: bold; margin-bottom: 6px; color: #334155;">Thời Gian Kết Thúc (*)</label>
        <input type="datetime-local" name="end_time" value="<?= isset($event) && $event->getEndTime() ? $event->getEndTime()->format('Y-m-d\TH:i') : '' ?>" required style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 8px; box-sizing: border-box;">
    </div>
</div>
